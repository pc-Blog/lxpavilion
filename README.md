...
**音乐**
- 独立 `/music` 页面：左侧按歌手浏览（含「全部」），右侧曲目列表，支持关键词搜索、分类筛选、只看收藏与分页，筛选条件变更自动回到第一页
- 正在播放的曲目可一键「定位歌曲」跳到所在页；上一首/下一首与自动切歌都在当前筛选范围内选曲
- 可拖动的悬浮播放器：封面 + 歌名/歌手、上一首 / 播放暂停 / 下一首、顺序 / 随机 / 单曲三种播放模式、进度拖拽、音量调节与静音，播放模式与音量本地记忆
- 首页音乐卡片与音乐页播放器共享播放状态，跳转页面不中断播放，点击卡片直达音乐页
- 本地部署下可直接添加/编辑歌曲、管理歌手与分类，线上静态模式为只读浏览
