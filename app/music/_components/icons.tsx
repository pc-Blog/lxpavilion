import type { SVGProps } from "react";

/**
 * Element Plus 图标（内联 SVG 版）。
 *
 * <p>音乐页的视觉基准是 OmniPavilion 音乐模块，那里用的是
 * {@code @element-plus/icons-vue}。该包是 Vue 组件，无法在 React 项目里直接使用，
 * 因此这里从它的 dist 中提取原始 path 数据（viewBox 0 0 1024 1024，
 * {@code fill="currentColor"}），重新包装成 React 组件，做到图形完全一致。</p>
 *
 * <p>用法与普通图标组件相同：{@code <VideoPlay size={20} />}，
 * 颜色沿用 {@code currentColor}。</p>
 */

type IconProps = SVGProps<SVGSVGElement> & { size?: number | string };

function Icon({
  paths,
  size = "1em",
  viewBox = "0 0 1024 1024",
  ...rest
}: IconProps & { paths: string[]; viewBox?: string }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox={viewBox}
      width={size}
      height={size}
      fill="currentColor"
      aria-hidden
      {...rest}
    >
      {paths.map((d, i) => (
        <path key={i} fill="currentColor" d={d} />
      ))}
    </svg>
  );
}

export function VideoPlay(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M512 64a448 448 0 1 1 0 896 448 448 0 0 1 0-896m0 832a384 384 0 0 0 0-768 384 384 0 0 0 0 768m-48-247.616L668.608 512 464 375.616zm10.624-342.656 249.472 166.336a48 48 0 0 1 0 79.872L474.624 718.272A48 48 0 0 1 400 678.336V345.6a48 48 0 0 1 74.624-39.936z",
      ]}
    />
  );
}

export function VideoPause(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M512 64a448 448 0 1 1 0 896 448 448 0 0 1 0-896m0 832a384 384 0 0 0 0-768 384 384 0 0 0 0 768m-96-544q32 0 32 32v256q0 32-32 32t-32-32V384q0-32 32-32m192 0q32 0 32 32v256q0 32-32 32t-32-32V384q0-32 32-32",
      ]}
    />
  );
}

export function Star(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "m512 747.84 228.16 119.936a6.4 6.4 0 0 0 9.28-6.72l-43.52-254.08 184.512-179.904a6.4 6.4 0 0 0-3.52-10.88l-255.104-37.12L517.76 147.904a6.4 6.4 0 0 0-11.52 0L392.192 379.072l-255.104 37.12a6.4 6.4 0 0 0-3.52 10.88L318.08 606.976l-43.584 254.08a6.4 6.4 0 0 0 9.28 6.72zM313.6 924.48a70.4 70.4 0 0 1-102.144-74.24l37.888-220.928L88.96 472.96A70.4 70.4 0 0 1 128 352.896l221.76-32.256 99.2-200.96a70.4 70.4 0 0 1 126.208 0l99.2 200.96 221.824 32.256a70.4 70.4 0 0 1 39.04 120.064L774.72 629.376l37.888 220.928a70.4 70.4 0 0 1-102.144 74.24L512 820.096l-198.4 104.32z",
      ]}
    />
  );
}

export function StarFilled(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M283.84 867.84 512 747.776l228.16 119.936a6.4 6.4 0 0 0 9.28-6.72l-43.52-254.08 184.512-179.904a6.4 6.4 0 0 0-3.52-10.88l-255.104-37.12L517.76 147.904a6.4 6.4 0 0 0-11.52 0L392.192 379.072l-255.104 37.12a6.4 6.4 0 0 0-3.52 10.88L318.08 606.976l-43.584 254.08a6.4 6.4 0 0 0 9.28 6.72z",
      ]}
    />
  );
}

export function Search(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "m795.904 750.72 124.992 124.928a32 32 0 0 1-45.248 45.248L750.656 795.904a416 416 0 1 1 45.248-45.248zM480 832a352 352 0 1 0 0-704 352 352 0 0 0 0 704",
      ]}
    />
  );
}

export function Plus(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M480 480V128a32 32 0 0 1 64 0v352h352a32 32 0 1 1 0 64H544v352a32 32 0 1 1-64 0V544H128a32 32 0 0 1 0-64z",
      ]}
    />
  );
}

export function Edit(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M832 512a32 32 0 1 1 64 0v352a32 32 0 0 1-32 32H160a32 32 0 0 1-32-32V160a32 32 0 0 1 32-32h352a32 32 0 0 1 0 64H192v640h640z",
        "m469.952 554.24 52.8-7.552L847.104 222.4a32 32 0 1 0-45.248-45.248L477.44 501.44l-7.552 52.8zm422.4-422.4a96 96 0 0 1 0 135.808l-331.84 331.84a32 32 0 0 1-18.112 9.088L436.8 623.68a32 32 0 0 1-36.224-36.224l15.104-105.6a32 32 0 0 1 9.024-18.112l331.904-331.84a96 96 0 0 1 135.744 0z",
      ]}
    />
  );
}

export function Delete(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M160 256H96a32 32 0 0 1 0-64h256V95.936a32 32 0 0 1 32-32h256a32 32 0 0 1 32 32V192h256a32 32 0 1 1 0 64h-64v672a32 32 0 0 1-32 32H192a32 32 0 0 1-32-32zm448-64v-64H416v64zM224 896h576V256H224zm192-128a32 32 0 0 1-32-32V416a32 32 0 0 1 64 0v320a32 32 0 0 1-32 32m192 0a32 32 0 0 1-32-32V416a32 32 0 0 1 64 0v320a32 32 0 0 1-32 32",
      ]}
    />
  );
}

export function Close(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M764.288 214.592 512 466.88 259.712 214.592a31.936 31.936 0 0 0-45.12 45.12L466.752 512 214.528 764.224a31.936 31.936 0 1 0 45.12 45.184L512 557.184l252.288 252.288a31.936 31.936 0 0 0 45.12-45.12L557.12 512.064l252.288-252.352a31.936 31.936 0 1 0-45.12-45.184z",
      ]}
    />
  );
}

export function CloseBold(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M195.2 195.2a64 64 0 0 1 90.496 0L512 421.504 738.304 195.2a64 64 0 0 1 90.496 90.496L602.496 512 828.8 738.304a64 64 0 0 1-90.496 90.496L512 602.496 285.696 828.8a64 64 0 0 1-90.496-90.496L421.504 512 195.2 285.696a64 64 0 0 1 0-90.496z",
      ]}
    />
  );
}

export function CircleClose(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "m466.752 512-90.496-90.496a32 32 0 0 1 45.248-45.248L512 466.752l90.496-90.496a32 32 0 1 1 45.248 45.248L557.248 512l90.496 90.496a32 32 0 1 1-45.248 45.248L512 557.248l-90.496 90.496a32 32 0 0 1-45.248-45.248z",
        "M512 896a384 384 0 1 0 0-768 384 384 0 0 0 0 768m0 64a448 448 0 1 1 0-896 448 448 0 0 1 0 896",
      ]}
    />
  );
}

export function Location(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M800 416a288 288 0 1 0-576 0c0 118.144 94.528 272.128 288 456.576C705.472 688.128 800 534.144 800 416M512 960C277.312 746.688 160 565.312 160 416a352 352 0 0 1 704 0c0 149.312-117.312 330.688-352 544",
        "M512 512a96 96 0 1 0 0-192 96 96 0 0 0 0 192m0 64a160 160 0 1 1 0-320 160 160 0 0 1 0 320",
      ]}
    />
  );
}

export function Picture(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M160 160v704h704V160zm-32-64h768a32 32 0 0 1 32 32v768a32 32 0 0 1-32 32H128a32 32 0 0 1-32-32V128a32 32 0 0 1 32-32",
        "M384 288q64 0 64 64t-64 64q-64 0-64-64t64-64M185.408 876.992l-50.816-38.912L350.72 556.032a96 96 0 0 1 134.592-17.856l1.856 1.472 122.88 99.136a32 32 0 0 0 44.992-4.864l216-269.888 49.92 39.936-215.808 269.824-.256.32a96 96 0 0 1-135.04 14.464l-122.88-99.072-.64-.512a32 32 0 0 0-44.8 5.952z",
      ]}
    />
  );
}

export function Headset(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M896 529.152V512a384 384 0 1 0-768 0v17.152A128 128 0 0 1 320 640v128a128 128 0 1 1-256 0V512a448 448 0 1 1 896 0v256a128 128 0 1 1-256 0V640a128 128 0 0 1 192-110.848M896 640a64 64 0 0 0-128 0v128a64 64 0 0 0 128 0zm-768 0v128a64 64 0 0 0 128 0V640a64 64 0 1 0-128 0",
      ]}
    />
  );
}

export function User(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M512 512a192 192 0 1 0 0-384 192 192 0 0 0 0 384m0 64a256 256 0 1 1 0-512 256 256 0 0 1 0 512m320 320v-96a96 96 0 0 0-96-96H288a96 96 0 0 0-96 96v96a32 32 0 1 1-64 0v-96a160 160 0 0 1 160-160h448a160 160 0 0 1 160 160v96a32 32 0 1 1-64 0",
      ]}
    />
  );
}

/*
 * 以下三个音量图标不来自 @element-plus/icons-vue，而是源项目
 * MusicPlayer.vue:14-18 用的 @element-extended-icon-pack/vue
 * （LinearNewSoundBroadcastVolumeUp / Down / Off）。
 * 该包的 viewBox 是 0 0 32 32（不是 1024），因此单独传 viewBox。
 */
export function VolumeUp(p: IconProps) {
  return (
    <Icon
      {...p}
      viewBox="0 0 32 32"
      paths={[
        "M14.293 2.293C14.923 1.663 16 2.109 16 3v26c0 .89-1.077 1.337-1.707.707L7.585 23H2a1 1 0 0 1-.993-.883L1 22V10a1 1 0 0 1 1-1h5.584zM14 5.414l-5.293 5.293A1 1 0 0 1 8 11H3v10h5a1 1 0 0 1 .608.206l.1.087L14 26.585zm5.536 7.05a5 5 0 0 1 0 7.072 1 1 0 0 1-1.415-1.415 3 3 0 0 0 0-4.242 1 1 0 0 1 1.415-1.415m7.07-7.07c5.858 5.857 5.858 15.355 0 21.213a1 1 0 1 1-1.414-1.415c5.077-5.076 5.077-13.308 0-18.384a1 1 0 0 1 1.415-1.415m-3.535 3.535c3.905 3.905 3.905 10.237 0 14.142a1 1 0 0 1-1.414-1.414 8 8 0 0 0 0-11.314A1 1 0 0 1 23.07 8.93",
      ]}
    />
  );
}

export function VolumeDown(p: IconProps) {
  return (
    <Icon
      {...p}
      viewBox="0 0 32 32"
      paths={[
        "M19.536 12.464a5 5 0 0 1 0 7.072 1 1 0 0 1-1.415-1.415 3 3 0 0 0 0-4.242 1 1 0 0 1 1.415-1.415M14.293 2.293C14.923 1.663 16 2.109 16 3v26c0 .89-1.077 1.337-1.707.707L7.585 23H2a1 1 0 0 1-.993-.883L1 22V10a1 1 0 0 1 1-1h5.584zM14 5.414l-5.293 5.293A1 1 0 0 1 8 11H3v10h5a1 1 0 0 1 .608.206l.1.087L14 26.585z",
      ]}
    />
  );
}

export function VolumeOff(p: IconProps) {
  return (
    <Icon
      {...p}
      viewBox="0 0 32 32"
      paths={[
        "M23 13.242V29c0 .89-1.077 1.337-1.707.707L14.585 23h-1.342l1.975-1.976a1 1 0 0 1 .39.182l.1.087L21 26.585V15.242zM23 3v4.584l3.293-3.291a1 1 0 0 1 1.414 1.414l-22 22a1 1 0 1 1-1.414-1.414l3.823-3.825a.995.995 0 0 1-.11-.351L8 22V10a1 1 0 0 1 1-1h5.584l6.709-6.707C21.923 1.663 23 2.109 23 3m-2 2.414-5.293 5.293A1 1 0 0 1 15 11h-5v9.584l11-11z",
      ]}
    />
  );
}

export function CaretLeft(p: IconProps) {
  return <Icon {...p} paths={["M672 192 288 511.936 672 832z"]} />;
}

export function CaretRight(p: IconProps) {
  return <Icon {...p} paths={["M384 192v640l384-320.064z"]} />;
}

export function Refresh(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M771.776 794.88A384 384 0 0 1 128 512h64a320 320 0 0 0 555.712 216.448H654.72a32 32 0 1 1 0-64h149.056a32 32 0 0 1 32 32v148.928a32 32 0 1 1-64 0v-50.56zM276.288 295.616h92.992a32 32 0 0 1 0 64H220.16a32 32 0 0 1-32-32V178.56a32 32 0 0 1 64 0v50.56A384 384 0 0 1 896.128 512h-64a320 320 0 0 0-555.776-216.384z",
      ]}
    />
  );
}

export function RefreshRight(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M784.512 230.272v-50.56a32 32 0 1 1 64 0v149.056a32 32 0 0 1-32 32H667.52a32 32 0 1 1 0-64h92.992A320 320 0 1 0 524.8 833.152a320 320 0 0 0 320-320h64a384 384 0 0 1-384 384 384 384 0 0 1-384-384 384 384 0 0 1 643.712-282.88z",
      ]}
    />
  );
}

export function Sort(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M384 96a32 32 0 0 1 64 0v786.752a32 32 0 0 1-54.592 22.656L95.936 608a32 32 0 0 1 0-45.312h.128a32 32 0 0 1 45.184 0L384 805.632zm192 45.248a32 32 0 0 1 54.592-22.592L928.064 416a32 32 0 0 1 0 45.312h-.128a32 32 0 0 1-45.184 0L640 218.496V928a32 32 0 1 1-64 0V141.248z",
      ]}
    />
  );
}

export function Setting(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M600.704 64a32 32 0 0 1 30.464 22.208l35.2 109.376c14.784 7.232 28.928 15.36 42.432 24.512l112.384-24.192a32 32 0 0 1 34.432 15.36L944.32 364.8a32 32 0 0 1-4.032 37.504l-77.12 85.12a357.12 357.12 0 0 1 0 49.024l77.12 85.248a32 32 0 0 1 4.032 37.504l-88.704 153.6a32 32 0 0 1-34.432 15.296L708.8 803.904c-13.44 9.088-27.648 17.28-42.368 24.512l-35.264 109.376A32 32 0 0 1 600.704 960H423.296a32 32 0 0 1-30.464-22.208L357.696 828.48a351.616 351.616 0 0 1-42.56-24.64l-112.32 24.256a32 32 0 0 1-34.432-15.36L79.68 659.2a32 32 0 0 1 4.032-37.504l77.12-85.248a357.12 357.12 0 0 1 0-48.896l-77.12-85.248A32 32 0 0 1 79.68 364.8l88.704-153.6a32 32 0 0 1 34.432-15.296l112.32 24.256c13.568-9.152 27.776-17.408 42.56-24.64l35.2-109.312A32 32 0 0 1 423.232 64H600.64zm-23.424 64H446.72l-36.352 113.088-24.512 11.968a294.113 294.113 0 0 0-34.816 20.096l-22.656 15.36-116.224-25.088-65.28 113.152 79.68 88.192-1.92 27.136a293.12 293.12 0 0 0 0 40.192l1.92 27.136-79.808 88.192 65.344 113.152 116.224-25.024 22.656 15.296a294.113 294.113 0 0 0 34.816 20.096l24.512 11.968L446.72 896h130.688l36.48-113.152 24.448-11.904a288.282 288.282 0 0 0 34.752-20.096l22.592-15.296 116.288 25.024 65.28-113.152-79.744-88.192 1.92-27.136a293.12 293.12 0 0 0 0-40.256l-1.92-27.136 79.808-88.128-65.344-113.152-116.288 24.96-22.592-15.232a287.616 287.616 0 0 0-34.752-20.096l-24.448-11.904L577.344 128zM512 320a192 192 0 1 1 0 384 192 192 0 0 1 0-384m0 64a128 128 0 1 0 0 256 128 128 0 0 0 0-256",
      ]}
    />
  );
}

export function Clock(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M512 896a384 384 0 1 0 0-768 384 384 0 0 0 0 768m0 64a448 448 0 1 1 0-896 448 448 0 0 1 0 896",
        "M480 256a32 32 0 0 1 32 32v256a32 32 0 0 1-64 0V288a32 32 0 0 1 32-32",
        "M480 512h256q32 0 32 32t-32 32H480q-32 0-32-32t32-32",
      ]}
    />
  );
}

export function DocumentChecked(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M805.504 320 640 154.496V320zM832 384H576V128H192v768h640zM160 64h480l256 256v608a32 32 0 0 1-32 32H160a32 32 0 0 1-32-32V96a32 32 0 0 1 32-32m318.4 582.144 180.992-180.992L704.64 510.4 478.4 736.64 320 578.304l45.248-45.312z",
      ]}
    />
  );
}

export function ArrowLeft(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M609.408 149.376 277.76 489.6a32 32 0 0 0 0 44.672l331.648 340.352a29.12 29.12 0 0 0 41.728 0 30.592 30.592 0 0 0 0-42.752L339.264 511.936l311.872-319.872a30.592 30.592 0 0 0 0-42.688 29.12 29.12 0 0 0-41.728 0z",
      ]}
    />
  );
}

export function ArrowRight(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M340.864 149.312a30.592 30.592 0 0 0 0 42.752L652.736 512 340.864 831.872a30.592 30.592 0 0 0 0 42.752 29.12 29.12 0 0 0 41.728 0L714.24 534.336a32 32 0 0 0 0-44.672L382.592 149.376a29.12 29.12 0 0 0-41.728 0z",
      ]}
    />
  );
}

export function ArrowDown(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M831.872 340.864 512 652.672 192.128 340.864a30.592 30.592 0 0 0-42.752 0 29.12 29.12 0 0 0 0 41.6L489.664 714.24a32 32 0 0 0 44.672 0l340.288-331.712a29.12 29.12 0 0 0 0-41.728 30.592 30.592 0 0 0-42.752 0z",
      ]}
    />
  );
}

export function MoreFilled(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M176 416a112 112 0 1 1 0 224 112 112 0 0 1 0-224m336 0a112 112 0 1 1 0 224 112 112 0 0 1 0-224m336 0a112 112 0 1 1 0 224 112 112 0 0 1 0-224",
      ]}
    />
  );
}

export function DArrowLeft(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M529.408 149.376a29.12 29.12 0 0 1 41.728 0 30.592 30.592 0 0 1 0 42.688L259.264 511.936l311.872 319.936a30.592 30.592 0 0 1-.512 43.264 29.12 29.12 0 0 1-41.216-.512L197.76 534.272a32 32 0 0 1 0-44.672l331.648-340.224zm256 0a29.12 29.12 0 0 1 41.728 0 30.592 30.592 0 0 1 0 42.688L515.264 511.936l311.872 319.936a30.592 30.592 0 0 1-.512 43.264 29.12 29.12 0 0 1-41.216-.512L453.76 534.272a32 32 0 0 1 0-44.672l331.648-340.224z",
      ]}
    />
  );
}

export function DArrowRight(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M452.864 149.312a29.12 29.12 0 0 1 41.728.064L826.24 489.664a32 32 0 0 1 0 44.672L494.592 874.624a29.12 29.12 0 0 1-41.728 0 30.592 30.592 0 0 1 0-42.752L764.736 512 452.864 192a30.592 30.592 0 0 1 0-42.688m-256 0a29.12 29.12 0 0 1 41.728.064L570.24 489.664a32 32 0 0 1 0 44.672L238.592 874.624a29.12 29.12 0 0 1-41.728 0 30.592 30.592 0 0 1 0-42.752L508.736 512 196.864 192a30.592 30.592 0 0 1 0-42.688z",
      ]}
    />
  );
}

export function InfoFilled(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M512 64a448 448 0 1 1 0 896.064A448 448 0 0 1 512 64m67.2 275.072c33.28 0 60.288-23.104 60.288-57.344s-27.072-57.344-60.288-57.344c-33.28 0-60.16 23.104-60.16 57.344s26.88 57.344 60.16 57.344M590.912 699.2c0-6.848 2.368-24.64 1.024-34.752l-52.608 60.544c-10.88 11.456-24.512 19.392-30.912 17.28a12.992 12.992 0 0 1-8.256-14.72l87.68-276.992c7.168-35.136-12.544-67.2-54.336-71.296-44.096 0-108.992 44.736-148.48 101.504 0 6.784-1.28 23.68.064 33.792l52.544-60.608c10.88-11.328 23.552-19.328 29.952-17.152a12.8 12.8 0 0 1 7.808 16.128L388.48 728.576c-10.048 32.256 8.96 63.872 55.04 71.04 67.84 0 107.904-43.648 147.456-100.416z",
      ]}
    />
  );
}

export function Loading(p: IconProps) {
  return (
    <Icon
      {...p}
      paths={[
        "M512 64a32 32 0 0 1 32 32v192a32 32 0 0 1-64 0V96a32 32 0 0 1 32-32m0 640a32 32 0 0 1 32 32v192a32 32 0 1 1-64 0V736a32 32 0 0 1 32-32m448-192a32 32 0 0 1-32 32H736a32 32 0 1 1 0-64h192a32 32 0 0 1 32 32m-640 0a32 32 0 0 1-32 32H96a32 32 0 0 1 0-64h192a32 32 0 0 1 32 32M195.2 195.2a32 32 0 0 1 45.248 0L376.32 331.008a32 32 0 0 1-45.248 45.248L195.2 240.448a32 32 0 0 1 0-45.248zm452.544 452.544a32 32 0 0 1 45.248 0L828.8 783.552a32 32 0 0 1-45.248 45.248L647.744 692.992a32 32 0 0 1 0-45.248zM828.8 195.264a32 32 0 0 1 0 45.184L692.992 376.32a32 32 0 0 1-45.248-45.248l135.808-135.808a32 32 0 0 1 45.248 0m-452.544 452.48a32 32 0 0 1 0 45.248L240.448 828.8a32 32 0 0 1-45.248-45.248l135.808-135.808a32 32 0 0 1 45.248 0z",
      ]}
    />
  );
}
